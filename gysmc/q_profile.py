    ...
    def __init__(
        ...
        if isinstance(option, str):
            ...
            if option_lower in option_map:
                ...
                raise ValueError(
                    f"Unknown q_profile option '{option}'. "
        ...
        if read_q:
            ...
            if option == 0:
                ...
                C2 = (rhomin6 * (q_param3 - q_param2) + (1 - rhomin2) ** 3 * (q_param1 - q_param2)) / var
                C3 = (rhomin4 * (q_param3 - q_param2) - (1 - rhomin2) ** 2 * (q_param1 - q_param2)) / var

                for ir in range(Nr + 1):
                    ri = self.grid_r[ir]
                    rho = ri / minor_radius
                    rho2 = rho**2
                    qr_tmp = q_param2 + C2 * (rho2 - rhomin2) ** 2 + C3 * (rho2 - rhomin2) ** 3
                    self.q_r[ir] = qr_tmp
