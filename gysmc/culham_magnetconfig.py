...
class CulhamMagnetConfig(MagnetConfig):
    ...
    def _compute_derivative(self, func_array):
        ...
        for i in range(2, nr - 2):
            derivative[i] = cdx * (
                func_array[i + 1] - func_array[i - 1] + (func_array[i - 2] - func_array[i + 2]) / 8.0
            )

        # Boundary points (4th order forward/backward difference)
        # Left boundary
        derivative[0] = (
            -25.0 / 12.0 * func_array[0]
            + 4.0 * func_array[1]
            - 3.0 * func_array[2]
            + 4.0 / 3.0 * func_array[3]
            - func_array[4] / 4.0
        ) / dr
        derivative[1] = (
            -func_array[0] / 4.0
            - 5.0 / 6.0 * func_array[1]
            + 3.0 / 2.0 * func_array[2]
            - 0.5 * func_array[3]
            + func_array[4] / 12.0
        ) / dr
        ...
        # Right boundary
        derivative[nr - 1] = (
            25.0 / 12.0 * func_array[nr - 1]
            - 4.0 * func_array[nr - 2]
            + 3.0 * func_array[nr - 3]
            - 4.0 / 3.0 * func_array[nr - 4]
            + func_array[nr - 5] / 4.0
        ) / dr
        derivative[nr - 2] = (
            func_array[nr - 1] / 4.0
            + 5.0 / 6.0 * func_array[nr - 2]
            - 3.0 / 2.0 * func_array[nr - 3]
            + 0.5 * func_array[nr - 4]
            - func_array[nr - 5] / 12.0
        ) / dr
    ...
    def _compute_g_function_RK4(self):
        ...
        solution = solve_ivp(g_ode, r_span, y0, method="RK45", t_eval=self.r_array, rtol=1e-10, atol=1e-12)
    ...
    def _compute_betap_profile(self):
        ...
        pressterm0 = self.pressterm_prime_r[0] - self.r_array[0] / dr * (
            self.pressterm_prime_r[1] - self.pressterm_prime_r[0]
        )
        ...
        for i in range(nr):
            ...
            for j in range(i + 1):
                ...
                pressterm_prime_tmp = self.pressterm_prime_r[j] - pressterm0 - pressterm1 * r_tmp
                integral_val += integr_elem * r_tmp**2 * pressterm_prime_tmp

            # Analytical part for constant and linear terms
            integral_val += pressterm0 * r_pos**3 / 3.0 + pressterm1 * r_pos**4 / 4.0
    ...
    def _integrate_ET_equation(self, coeff_0, init_u, init_v):
        ...
        # Solve the ODE system
        r_span = (self.r_array[0], self.r_array[-1])
        solution = solve_ivp(ode_system, r_span, y0, method="RK45", t_eval=self.r_array, rtol=1e-8, atol=1e-10)
        ...
        if abs(self.delta_triangularity) < 1e-3:
            ...
            def equations(x):
                ...
                fE = (
                    (1.0 + E_a - Pfunc_a)
                    / (1.0 - E_a - Pfunc_a)
                    * (1.0 - 2.0 * T_a / (1.0 + E_a - Pfunc_a) * cos_thz)
                    * sin_thz
                )
                fT = T_a / (1.0 - E_a - Pfunc_a) - (1.0 + (1.0 + E_a - Pfunc_a) / (1.0 - E_a - Pfunc_a) / 2.0) * cos_thz
    ...
    def _compute_dR_dZ_transformation(self, r, theta):
        ...
        dR_dr = Deltaprime_pos + cos_theta * (1.0 - Eprime_pos - Pfuncprime_pos) + Tprime_pos * cos_2theta

        # dR/dtheta = -sin(theta) * (r - E - P) - 2*T * sin(2*theta)
        dR_dtheta = -sin_theta * (r_bc - E_pos - Pfunc_pos) - 2.0 * T_pos * sin_2theta

        # dZ/dr = sin(theta) * (1 + E' - P') - T' * sin(2*theta)
        dZ_dr = sin_theta * (1.0 + Eprime_pos - Pfuncprime_pos) - Tprime_pos * sin_2theta

        # dZ/dtheta = cos(theta) * (r + E - P) - 2*T * cos(2*theta)
        dZ_dtheta = cos_theta * (r_bc + E_pos - Pfunc_pos) - 2.0 * T_pos * cos_2theta
    ...
    def _get_jacobian_theta_thetastar_(self, r, thetastar, phi):
        ...
        dterm1_dr = (
            -Deltaprimeprime_pos * (1.0 + E_pos / r)
        ...
        dterm3_dr = (
            (1.0 / 3.0) * (Tprimeprime_pos - 2.0 * Tprime_pos / r + 2.0 * T_pos / r**2)[:, None] * np.sin(3 * theta)
        )

        jacobian[..., 1, 0] = (-dterm1_dr - dterm2_dr + dterm3_dr)[:, :, None] + phi[
            None, None, :
        ] * 0.0  # broadcast to match shape
        ...
        jacobian[..., 1, 1] = (1.0 - dterm1_dtheta - dterm2_dtheta + dterm3_dtheta)[:, :, None] + phi[
            None, None, :
        ] * 0.0
    ...
    def get_RZ(self, tor1_arr, tor2_arr, tor3_arr):
        ...
        R_2d = self.R0 + r_bc * cos_theta + Delta_bc - E_bc * cos_theta + T_bc * cos_2theta - Pfunc_bc * cos_theta

        # Z = r*sin(theta) + E*sin(theta) - T*sin(2*theta) - P*sin(theta)
        Z_2d = r_bc * sin_theta + E_bc * sin_theta - T_bc * sin_2theta - Pfunc_bc * sin_theta
    ...
    def get_Jcontra(self, tor1_arr, tor2_arr, tor3_arr):
        ...
        for i, r_pos in enumerate(tor1_arr):
            ...
            for j, theta_pos in enumerate(tor2_arr):
                for k in range(nb_grid_tor3):
                    tor3_arr if nb_grid_tor3 == 1 else tor3_arr[k]
                    ...
                    J_contra[i, j, k, 2] = -(dIdr * B_contra[i, j, k, 2] + pressterm_prime_pos) / dpsidr
